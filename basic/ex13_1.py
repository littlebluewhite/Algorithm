...
def search_dealer(name):
    already_search_list = []
    dealer = deque()
    dealer.append(name)
    while dealer:
        print(f" 目前搜尋名單: {dealer}")
        person = dealer.popleft()
        if person == "Banana":
            print(f"{person} is dealer!")
            print(f"已搜尋名單: {already_search_list}")
            return
        else:
            print(f"{person} is not dealer")
            already_search_list.append(person)
            dealer += graph[person]
